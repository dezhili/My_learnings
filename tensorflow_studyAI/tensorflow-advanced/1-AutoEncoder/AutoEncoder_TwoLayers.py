...
#网络模型参数
n_hidden1_units = 256  # 隐藏层神经元数量(让编码器和解码器有同样规模的隐藏层)
n_hidden2_units = 128
n_input_units = 784
...
# 根据输入输出节点数量返回初始化好的指定名称的偏置Variable
...
with tf.Graph().as_default():
    ...
    print('把计算图写入事件文件， 在Tensorboard 里面查看')
    summary_writer = tf.summary.FileWriter(logdir='F:/Learnings/All_learnings/Learning/tensorflow_studyAI/tensorflow-advanced/1-AutoEncoder/logs2',
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(training_epochs):
            # 每一轮都需要把所有的 batch 跑一遍
            for i in range(total_batch):
                batch_xs, batch_ys = mnist.train.next_batch(batch_size)
                # 运行优化器Train节点(bachpropagation) 和 Loss节点(获取损失值)
        ...
        # 把训练好的编码器-解码器模型用在测试集，输出重建后的样本数据
