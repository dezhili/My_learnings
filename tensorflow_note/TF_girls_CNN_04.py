...
class Network():
	def __init__(self, num_hidden, batch_size, patch_size, conv_depth, pooling_scale):
		...
		self.pooling_stride = pooling_scale  # 10x10 pooling_stride = 2(2^2) --> 5x5  2^2=pooling_stride**2
		...
		self.define_graph()
		writer = tf.train.SummaryWriter('TF-graph',self.graph)
	...
	def define_graph(self):
		...
		with self.graph.as_default():
			...
			# fully connected layer 1, fully connected(需要reshape 扁平化后computing)
			with tf.name_scope('fc1'):
				down_scale = self.pooling_stride ** 2

				fc1_weights = tf.Variable(
					tf.truncated_normal(
						[(image_size // down_scale) * (image_size//down_scale) *(self.last_conv_depth), 
							self.num_hidden], stddev=0.1),name='fc1_weights')
		...
		with self.session as session:
			...
			for i, samples, labels in get_chunk(train_samples, train_labels, chunkSize=self.batch_size):
				_, l, predictions ,summary = session.run(
					[self.optimizer, self.loss, self.train_prediction, merge],
...
if __name__ == '__main__':
	...
	#  本次使用的架构 Conv3x3 Relu    Conv3x3 Relu MaxPool2 ---> 
	#  				 Conv3x3 Relu    Conv3x3 Relu MaxPool2 --->
